...
export default async function pdfTldr() {
  const pdfFilePath =
    "assets/AAPL_FY23_Q1_Consolidated_Financial_Statements.pdf";
  const pdfReader = new PdfReader(pdfFilePath);

  const question = "What is the total net sales for the quarter in 2022?";
  const answer = await pdfReader.answerQuestion(
    "What is the total net sales for the quarter in 2022?",
  );
